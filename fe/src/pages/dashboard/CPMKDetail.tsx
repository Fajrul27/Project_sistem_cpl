                ...
                <div className="space-y-6 min-w-0">
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        <Card className="hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer">
                            ...
                            <CardContent>
                                <div className="text-lg font-bold leading-tight truncate">{cpmk.kodeCpmk}</div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer">
                            ...
                            <CardContent>
                                <div className="text-lg font-bold leading-tight">
                                    {cpmk.levelTaksonomi ? `${cpmk.levelTaksonomi} - ${getLevelTaksonomiDeskripsi(cpmk.levelTaksonomi)}` : "-"}
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <DialogContent>
                            ...
                            <form onSubmit={handleSubmitMapping} className="space-y-4">
                                <div className="space-y-2">
                                    <Label>CPL</Label>
                                    {editingMapping ? (
                                        <Input value={`${editingMapping.cpl.kodeCpl} - ${editingMapping.cpl.deskripsi}`} disabled />
                                    ) : (
                                        <Select
                                            value={mappingForm.cplId}
                                            onValueChange={(val) => setMappingForm({ ...mappingForm, cplId: val })}
                                        >
                                            <SelectTrigger><SelectValue placeholder="Pilih CPL" /></SelectTrigger>
                                            <SelectContent>
                                                {unmappedCpl.map(c => (
                                                    <SelectItem key={c.id} value={c.id}>
                                                        {c.kodeCpl} - {c.deskripsi.length > 50 ? c.deskripsi.substring(0, 50) + "..." : c.deskripsi}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
