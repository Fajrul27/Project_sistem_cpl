/*
  ...
  - Added the required column `prodi_id` to the `cpl` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE `cpl` DROP COLUMN `bobot`,
    ADD COLUMN `kategori_id` VARCHAR(191) NULL,
    ADD COLUMN `minimal_nilai_tercapai` DECIMAL(5, 2) NOT NULL DEFAULT 70.00,
    ADD COLUMN `prodi_id` VARCHAR(191) NOT NULL;
...
CREATE TABLE `prodi` (
    ...
    UNIQUE INDEX `prodi_kode_key`(`kode`),
...
-- AddForeignKey
ALTER TABLE `cpl` ADD CONSTRAINT `cpl_prodi_id_fkey` FOREIGN KEY (`prodi_id`) REFERENCES `prodi`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
