...
CREATE TABLE `visi_misi` (
    ...
    INDEX `visi_misi_prodi_id_idx`(`prodi_id`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `profil_lulusan` (
    ...
    INDEX `profil_lulusan_prodi_id_idx`(`prodi_id`),
    UNIQUE INDEX `profil_lulusan_prodi_id_kode_key`(`prodi_id`, `kode`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `visi_misi` ADD CONSTRAINT `visi_misi_prodi_id_fkey` FOREIGN KEY (`prodi_id`) REFERENCES `prodi`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `profil_lulusan` ADD CONSTRAINT `profil_lulusan_prodi_id_fkey` FOREIGN KEY (`prodi_id`) REFERENCES `prodi`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
