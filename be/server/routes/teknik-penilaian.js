...
router.get('/cpmk/:cpmkId', authMiddleware, async (req, res) => {
    try {
        ...
        const totalBobot = teknikPenilaian.reduce((sum, tp) => sum + Number(tp.bobotPersentase), 0);
        res.json({
...
async function recalculateCpmkForBatch(cpmkId, mataKuliahId) {
    try {
        ...
        for (const grade of affectedGrades) {
            await calculateNilaiCpmk(grade.mahasiswaId, cpmkId, mataKuliahId, grade.semester, grade.tahunAjaran);
        }
    }
    catch (error) {
