...
router.get('/cpmk/:cpmkId', authMiddleware, async (req, res) => {
    try {
        ...
        const totalBobot = mappings.reduce((sum, mapping) => sum + Number(mapping.bobotPersentase), 0);
        res.json({
            data: mappings,
...
async function recalculateCplForBatch(cpmkId, mataKuliahId) {
    try {
        ...
        for (const grade of affectedGrades) {
            await calculateNilaiCplFromCpmk(grade.mahasiswaId, mataKuliahId, grade.semester, grade.tahunAjaran);
        }
    }
    catch (error) {
