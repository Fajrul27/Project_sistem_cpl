...
router.get('/cpmk/:cpmkId/:mahasiswaId', authMiddleware, async (req, res) => {
    try {
        ...
        const nilaiTeknik = await Promise.all(teknikList.map(async (teknik) => {
            const where = {
                mahasiswaId,
...
router.put('/:id', authMiddleware, requireRole('admin', 'dosen'), async (req, res) => {
    try {
        ...
        // Recalculate CPMK
        await calculateNilaiCpmk(existing.mahasiswaId, existing.teknikPenilaian.cpmkId, existing.mataKuliahId, existing.semester, existing.tahunAjaran);
        res.json({
            data: updated,
