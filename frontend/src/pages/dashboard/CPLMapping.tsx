  ...
  const handleBatchMapping = async () => {
    ...
    try {
      ...
      const newMappings = Array.from(selectedCPLs).map(cplId => ({
        ...
        bobotKontribusi: bobot[cplId] ? parseFloat(bobot[cplId].toString()) : 1.0,
      }));

      // Validasi total bobot
      const totalBobot = newMappings.reduce((sum, item) => sum + item.bobotKontribusi, 0);
      if (Math.abs(totalBobot - 1) > 0.01) {
        const confirmProceed = confirm(`Total bobot saat ini adalah ${totalBobot.toFixed(2)}. Lanjutkan?`);
        if (!confirmProceed) {
          setSubmitting(false);
          return;
        }
